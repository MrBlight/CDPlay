"""
cdplayer.py  -  terminal CD player & ripper for Windows  (v1.16.1)
Multi-platform edition - Windows Version

Playback via MCI (built-in Windows).
Ripping via SCSI READ CD or ffmpeg cdda/libcdio.
...
Debug flags:
  --win-debug   Bypass OS check (for testing Windows code on other OSes)
"""
...
if '--win-debug' in sys.argv:
    DEBUG_BYPASS = True
    sys.argv.remove('--win-debug')

if not DEBUG_BYPASS:
    if sys.platform != 'win32':
        print(f"\n[ERROR] You are running the Windows version on '{sys.platform}'.")
        print("This version (Win-Ver.py) is designed for Microsoft Windows only.")
        print("")
        print("Please use:")
        print("  • LB-Ver.py     for Linux-based systems")
        print("  • Mac-Ver.py    for macOS")
        print("  • BSD-Ver.py    for FreeBSD/OpenBSD/NetBSD")
        print("")
        print("Run with --win-debug to bypass this check (for development).")
        sys.exit(1)
else:
    # Debug mode - warn but continue
    print(f"[DEBUG] Bypassing OS check. Running Windows code on {sys.platform}.")
    print("[DEBUG] Expect failures when calling Windows-specific APIs.\n")

# Note: The following import will fail on non-Windows even in debug mode
# This is expected behavior for deep debugging
try:
    k32   = ctypes.windll.kernel32
    winmm = ctypes.windll.winmm
except AttributeError:
    if not DEBUG_BYPASS:
        sys.exit("[error] Windows only - cannot load Windows DLLs")
    else:
        # In debug mode, create stub objects to allow loading
        class StubDLL:
            def __getattr__(self, name):
                raise OSError(f"[DEBUG] Windows DLL function '{name}' not available on {sys.platform}")
        k32 = StubDLL()
        winmm = StubDLL()
